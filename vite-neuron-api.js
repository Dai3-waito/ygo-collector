  ...
  return {
    ...
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        ...
        if (url.startsWith('/api/ygo-neuron-card')) {
          const cid = new URL(url, 'http://local').searchParams.get('cid')
          if (!cid) {
            res.statusCode = 400
            res.end(JSON.stringify({ error: 'cid is required' }))
            return
          }
          try {
            const cardUrl = neuronCardPageUrl(cid)
            ...
            res.end(
              JSON.stringify({
                cid,
                url: cardUrl,
                prints: parseNeuronCardPrints(html),
              }),
            )
          } catch {
            res.statusCode = 502
